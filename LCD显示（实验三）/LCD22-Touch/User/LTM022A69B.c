...
void lcd_ctrl_port_init()
{
	...
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_12;
...
void lcd_init(void)
{
  ...
  SPI0_Init();
	...
	lcd_clear_screen(0XFFFF);//此处将背景改成白色，此函数为lcd初始化函数
}
...
* Return         : None
******************************************************************************/
void lcd_display_char(  unsigned char ch_asc,           //函数功能 写一个字符
                        unsigned int color_front,											//参数分别为 ascii表上第n个字符 字体颜色 字体背景 字符坐标x轴 字符坐标y轴
                        ...
                        unsigned char postion_y)
{
	...
	p = ascii;                  //定义指针指向ascii表首地址
	p += ch_asc*16;				//16个字节为一组
	for(j=0;j<16;j++)				//for循环以16个字节为一组写字符到LCD
	{
		b=*(p+j);
		for(i=0;i<8;i++)
		{
			if( b & 0x80 )
			{
				LCD_WRITE_DATA(color_front);      //SPI方式写入数据
			}
			else
			{
				LCD_WRITE_DATA(color_background);
...
* Return         : None
******************************************************************************/
void lcd_display_string(unsigned char *str,      //函数功能 写字符串
						unsigned int color_front, 										//函数参数 字符串 字体颜色 字体背景 首字符放置坐标
						unsigned int color_background, 
						unsigned char x, 
						unsigned char y )
{
	while (*str) //判断 字符不为空时 执行以下代码
	{ 
		lcd_display_char( *str, color_front, color_background, x, y);//调用单个字符显示函数显示字符串
		if(++x>=30)
		{
			x=0;
			if(++y>=20)
...
*/
void lcd_display_GB2312( unsigned char gb,         //函数功能 显示一个常规汉字16x16
						unsigned int color_front, 													//参数 存放汉字数组中第n个汉字 汉字颜色 汉字背景 汉字坐标
						unsigned int color_background, 
						unsigned char postion_x, 
						unsigned char postion_y )
{
	...
	p = (unsigned char *)GB2312;      //指针指向储存汉字的数组
	p += gb*32;																	//32个字节为一个汉字
	for(j=0;j<32;j++)                          //以32个字节为一组以SPI方式写入LCD
	{
		b=*(p+j);
		for(i=0;i<8;i++)
		{
...
void lcd_display_32x29( unsigned char gb,           //显示二号字体32x29 参数同上
						...
						unsigned char postion_y )
{
	...
	LCD_WRITE_COMMAND(0x210,postion_x*32); 	//x start point
	LCD_WRITE_COMMAND(0x212,postion_y*29); 	//y start point
	LCD_WRITE_COMMAND(0x211,postion_x*32+31);	//x end point
	LCD_WRITE_COMMAND(0x213,postion_y*29+28);	//y end point
	LCD_WRITE_COMMAND(0x200,postion_x*32);	
	LCD_WRITE_COMMAND(0x201,postion_y*29);

	LCD_RS_L();
	LCD_WRITE_REG(0x202);	//RAM Write index
	LCD_CS_L();
	
	p = (unsigned char *)GB3229;
	p += gb*116;                 //字体越大 占用的字节数越多 此处占用116个字节
	for(j=0;j<116;j++)
	{
		b=*(p+j);
		for(i=0;i<8;i++)
                  ...
                  unsigned int y)
{
	...
	LCD_WRITE_COMMAND(0x211,x);
	LCD_WRITE_COMMAND(0x213,y);
